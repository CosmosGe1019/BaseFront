  ...
  <div class="z-50">
    <div class="relative" ref="containerRef">
      <nav class="flex relative" :style="{ transform: 'translate3d(0,0,0.01px)' }">
        <ul
            ref="navRef"
            class="flex gap-8 list-none p-0 px-4 font-medium m-0 relative z-[3]"
            :style="{
            color: 'red',
          }"
        >
          <li
              ...
              :key="index"
              :class="[
              'rounded-full relative cursor-pointer duration-300 ease text-[var(--theme-main)]',
          ...
          >
            <RouterLink
                ...
                class="outline-none py-[0.6em] px-[1em] inline-block cursor-target"
            >
              {{ item.label }}
            </RouterLink>
          </li>
...
/* 只有当蓝色胶囊背景处于激活态时，文字才变为白色 */
.effect.filter.active + .effect.text {
  color: #ffffff;
}
